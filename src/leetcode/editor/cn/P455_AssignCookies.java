...
/******************************* 题目思路 *******************************/
// 利用贪心算法，无非就是不要造成大饼干的浪费。所以，直接按照饼干从大到小的顺序，按胃口从大到小的顺序分配给每个孩子即可；
...
class P455_AssignCookies {
    ...
    class Solution {
        public int findContentChildren(int[] g, int[] s) {
            ...
            int sindex = slen - 1;  // 记录分配到那块饼干了
            int result = 0;     // 满足了几个孩子

            for (int i = glen - 1; i >= 0; i--) {    // 将饼干分配给小孩，所以遍历小孩
                if (sindex >= 0 && s[sindex] >= g[i]) {     // 有饼干，能满足胃口，就分给它
                    result++;
                    sindex--;
                }
            }
